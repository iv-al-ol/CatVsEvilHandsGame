...
def add_image(add_image_list, folder_img, quantity_img=None):
    ...
    if (quantity_img != None):
        for num in range(quantity_img):
            ...
            added_img_list.append(pg.image.load
                                  (os.path.join(folder_img, filename))
                                  .convert_alpha())
    else:
        for img in add_image_list:    
            added_img_list.append(pg.image.load
                                  (os.path.join(folder_img, img))
                                  .convert_alpha())
...
def add_and_scale_image(img_name, folder_img, quantity_img, zoom_sise):
    ...
    for num in range(quantity_img):
        ...
        img_transform = pg.transform.scale(img, (img.get_width()*zoom_sise // 100,
                                                 img.get_height()*zoom_sise // 100))
...
class Cat(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.direction = 'move_down'  # Начальное направление движения
        self.image = img_cat_player[self.direction][1]  # Изображение персонажа
        self.rect = self.image.get_rect()
        self.rect.centerx = WIDTH // 2  # Точка появления персонажа
        self.rect.centery = HEIGHT // 2  # Точка появления персонажа
        
        self.speed_x = 0  # 
        self.speed_y = 0  # 
        
        self.move_speed_x = 6  # Скорость перемещения по 'x'
        self.move_speed_y = 6  # Скорость перемещения по 'y'

        self.last_center = self.rect.center  # Последний центр персонажа
        
        self.health = 100  # Здоровье
        self.lives = 3  # Жизни
        self.hidden = False  # Скрытое состояние персонажа
        self.rebirth_time = 2200  # Время перерождения
        self.hidden_time = pg.time.get_ticks()  # Время скрытия персонажа
        self.player_recovery_add = False  # Добавление анимации восстановления
        
        self.shoot_delay = 200  # Задержка на стрельбу
        self.last_shot = pg.time.get_ticks()  # Время последнего выстрела
        self.shoot_speed_x = 0  # Скорость выстрела по оси 'x'
        self.shoot_speed_y = 0  # Скорость выстрела по оси 'y'

        self.frame = 0  # Начальный кадр
        self.last_update = pg.time.get_ticks()  # Время от последнего обновления кадра
        self.frame_rate = 60  # Частота обновления кадров
    ...
    def update(self):
        ...
        def rebirth():
            ...
            if (self.hidden):
                if (not self.player_recovery_add and
                    not player_death_explosion.alive() and 
                    (pg.time.get_ticks() - player_cat.hidden_time) > 
                    self.rebirth_time):
                        player_recovery = BloodExplosion(player_cat.last_center,
                                                         'large', 100, 1, True)
                        all_sprites.add(player_recovery)
                        self.player_recovery_add = True
                        player_cat.hidden_time = pg.time.get_ticks()
                if (self.hidden and not player_death_explosion.alive() and
                    (pg.time.get_ticks() - player_cat.hidden_time) >
                    self.rebirth_time):
                        self.player_recovery_add = False
                        self.hidden = False
                        self.health = 100
...
class BloodExplosion(pg.sprite.Sprite):
    ...
    invert: Позволяет запустить отображение кадров в обраную сторону;
    
    """
    
    def __init__(self, center, size, frame_rate, anim_type, invert):
        pg.sprite.Sprite.__init__(self)
        self.size = size
        self.anim_type = anim_type
        self.image = img_blood_anim_1[self.size][-1]
        self.rect = self.image.get_rect()
        self.rect.center = center
        self.frame = 0
        self.last_update = pg.time.get_ticks()
    ...
    def update(self):
        def anim_choice(len_calc, frame, invert):
            ...
            if len_calc == False:    
                if invert == False:
                    ...
                    if self.anim_type == 1:
                        img_blood_anim_1_revrs = img_blood_anim_1[self.size][::-1]
                        self.image = img_blood_anim_1_revrs[frame]
                    elif self.anim_type == 2:
                        img_blood_anim_2_revrs = img_blood_anim_2[self.size][::-1]
                        self.image = img_blood_anim_2_revrs[frame]
                    elif self.anim_type == 3:
                        img_blood_anim_3_revrs = img_blood_anim_3[self.size][::-1]
                        self.image = img_blood_anim_3_revrs[frame]
                    elif self.anim_type == 4:
                        img_blood_anim_4_revrs = img_blood_anim_4[self.size][::-1]
                        self.image = img_blood_anim_4_revrs[frame]
                    elif self.anim_type == 5:
                        img_blood_anim_5_revrs = img_blood_anim_5[self.size][::-1]
                        self.image = img_blood_anim_5_revrs[frame]
                    else:
                        img_blood_anim_1_revrs = img_blood_anim_1[self.size][::-1]
                        self.image = img_blood_anim_1_revrs[frame]
                return self.image
        ...
        def frame_updater():
            ...
            if ((now - self.last_update) > self.frame_rate):
                self.last_update = now
                if self.frame == len(anim_choice(True, 0, self.invert)) - 1:
                    player_cat.hidden_time = pg.time.get_ticks()
                    player_recovery_add = False
                    self.kill()
                else:
                    center = self.rect.center
                    self.image = anim_choice(False, self.frame, self.invert)
                    self.rect = self.image.get_rect()
                    self.rect.center = center
                    self.frame += 1
        frame_updater()
...
while running:
    ...
    for hit in hits_player_hands:
        player_cat.health -= abs(hit.rot_speed) + \
                             abs(hit.speed_x) + abs(hit.speed_y)
        ...
        if player_cat.health <= 0:
            ...
            player_death_explosion = \
                BloodExplosion(player_cat.last_center, 
                               'large', 90, 1, False)
            all_sprites.add(player_death_explosion)
    ...
    draw_text(screen, str('Уничтожено рук: %s' % score), 30,
              WIDTH // 2, HEIGHT // 90)
    draw_health_bar(screen, WIDTH // 2, HEIGHT - HEIGHT // 11,
                    player_cat.health, RED)
    draw_text(screen, str('Здоровье: %s' % player_cat.health), 30, 
              WIDTH // 2, HEIGHT - HEIGHT // 16)
    draw_lives(screen, WIDTH // 1.4, HEIGHT - HEIGHT // 19,
               player_cat.lives, img_cat_player_lives)
